...
workpath='/gdelt/'
writelogs='/gdelt/mention_writelogs.txt'

get_url() {
    local result=$(wget -qO - http://data.gdeltproject.org/gdeltv2/lastupdate.txt | awk 'FNR == 2 {print}' | awk {'print $3'})
    echo $result
}
...
do
    ...
    target="s3://gdelt-tharid/mention/$(date +'%Y%m%d')/"
